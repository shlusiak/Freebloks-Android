...
@Composable
private fun NameDialog(initialValue: String, onChange: (String) -> Unit, onDismiss: () -> Unit) {
    var value by rememberSaveable { mutableStateOf(initialValue) }

    AlertDialog(
        onDismissRequest = onDismiss,
        title = { Text(stringResource(R.string.prefs_player_name)) },
        text = {
            OutlinedTextField(
                value = value,
                ...
                shape = CircleShape,
                singleLine = true,
                placeholder = { Text(stringResource(R.string.player_name_hint)) }
            )
        },
        confirmButton = {
            TextButton(onClick = {
